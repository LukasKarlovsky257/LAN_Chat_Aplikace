...
const admin = {
    ...
    mute: (u) => {
        ModernDialog.showInput("Mute Hráče", "Sekundy (-1 navždy):").then(d => {
            if(d) net.sendText(`/mute ${u} ${d}`);
        });
    },
    banModal: (u) => { state.targetUser = u; document.getElementById('ban-target-name').innerText = u; document.getElementById('modal-ban').style.display = 'flex'; },
    confirmBan: () => { const t = document.getElementById('ban-duration').value || 60; const r = document.getElementById('ban-reason').value; net.sendText(`/ban ${state.targetUser} ${t} ${r}`); ui.closeModals(); },
    deleteRoom: (r) => {
...
const ctx = {
    ...
    // 🔥 Přidána nová funkce pro chytrou kalkulaci pozice
    adjustPosition: (e) => {
        ...
        // Pokud by menu vyteklo zprava, posuneme ho doleva od kurzoru
        if (left + menuWidth > windowWidth) {
            left = windowWidth - menuWidth - 10;
        }

        // Pokud by menu vyteklo dole, vykreslíme ho NAD kurzorem
        if (top + menuHeight > windowHeight) {
            top = e.pageY - menuHeight;
            // Záchrana pro extrémně malá okna
            if (top < 0) top = 10;
        }
    ...
    showSide: (e, u) => {
        ...
        ctx.adjustPosition(e); // Použití chytré pozice
    },
