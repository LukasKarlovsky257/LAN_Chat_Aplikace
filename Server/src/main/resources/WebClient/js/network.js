...
const net = {
    sendText: (text) => {
        ...
        if (text.startsWith("GAME:") || text.startsWith("IMG:") || text.startsWith("FILE:") || text.includes("ZK:") || text.startsWith("SET_AVATAR:") || text.startsWith("ROOM_KEY_")) {
            state.ws.send(text);
...
const protocol = {
    handle: (msg) => {
        ...
        if (msg.startsWith("ROOM_KEY_REQ:")) {
            const parts = msg.substring(13).split(":", 3);
            if (parts.length === 3) {
                const requesterName = parts[0];
                const roomName = parts[1];
                const requesterPubKey = parts[2];

                const myAesKey = state.roomKeys[roomName];
                if (myAesKey) {
                    const encryptor = new JSEncrypt();
                    encryptor.setPublicKey(requesterPubKey);
                    const encryptedAesKey = encryptor.encrypt(myAesKey);

                    if (encryptedAesKey) {
                        net.sendText("ROOM_KEY_RES:" + requesterName + ":" + encryptedAesKey);
                        console.log(`🔐 Poslán zašifrovaný klíč uživateli ${requesterName}`);
                    }
                }
            }
            return;
        ...
        if (msg.startsWith("ROOM_KEY_RES:")) {
            const parts = msg.substring(13).split(":", 2);
            if (parts.length === 2) {
                const roomName = parts[0];
                const encryptedAesKey = parts[1];

                const decryptedAesKey = state.myRsa.decrypt(encryptedAesKey);
                if (decryptedAesKey) {
                    state.roomKeys[roomName] = decryptedAesKey;
                    console.log(`✅ Klíč pro místnost ${roomName} úspěšně přijat a rozšifrován!`);

                    const chatBox = document.getElementById('messages-box');
                    if(chatBox) {
                        chatBox.innerHTML += `<div style="text-align:center; padding:5px; color:var(--neon-purple); font-size:12px; font-weight:bold; margin-top: 10px;">🔒 Zabezpečené spojení navázáno (RSA-2048/AES-256)</div>`;
                        chatBox.scrollTop = chatBox.scrollHeight;
                    }
                } else {
                    console.error("❌ Nepodařilo se dešifrovat AES klíč od hostitele.");
                }
            }
            return;
        }
        ...
        if (msg.startsWith("LOGIN_OK:")) {
            ...
            ui.updateRoomUI(newRoom);

            if (newRoom !== "Lobby" && !state.roomKeys[newRoom]) {
                net.sendText("ROOM_KEY_REQ:" + newRoom + ":" + state.myPublicKey);
            }
        }
        ...
        else if (msg.startsWith("MUTE_START:")) {
            const seconds = parseInt(msg.split(":")[1]);
            chat.setMuted(seconds);
            return;
        }
