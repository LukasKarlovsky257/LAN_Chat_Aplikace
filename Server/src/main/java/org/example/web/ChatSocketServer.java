...
public class ChatSocketServer extends WebSocketServer {
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        if (message.startsWith("MSG:") || message.startsWith("ZK:") || message.startsWith("FILE:") || message.startsWith("IMG:")) {
            if (DatabaseManager.isBanned(nick)) {
                conn.send("MSG:0:SYSTEM:⛔ Jsi zabanován a nemůžeš odesílat zprávy.");
                return;
            }
            if (DatabaseManager.isMuted(nick)) {
                long remain = DatabaseManager.getMuteRemaining(nick);
                conn.send("MSG:0:SYSTEM:⛔ Jsi umlčen. Zbývá: " + remain + " sekund.");
                return;
            }
        }
        ...
        if (message.startsWith("ROOM_KEY_REQ:")) {
            String[] parts = message.substring(13).split(":", 2);
            if (parts.length == 2) {
                String targetRoomName = parts[0];
                String publicKey = parts[1];

                if (Server.privateRooms.containsKey(targetRoomName)) {
                    String hostName = Server.privateRooms.get(targetRoomName).host;
                    Server.relayKeyExchange(nick, hostName, "ROOM_KEY_REQ:" + nick + ":" + targetRoomName + ":" + publicKey);
                }
            }
            return;
        }

        if (message.startsWith("ROOM_KEY_RES:")) {
            String[] parts = message.substring(13).split(":", 2);
            if (parts.length == 2) {
                String targetPlayer = parts[0];
                String encryptedAesKey = parts[1];
                Server.relayKeyExchange(nick, targetPlayer, "ROOM_KEY_RES:" + currentRoom + ":" + encryptedAesKey);
            }
            return;
        }
        ...
        if (message.startsWith("/delmsg ")) {
            if (DatabaseManager.isAdmin(nick)) {
                ...
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva na mazání zpráv.");
            }
            return;
        }
        ...
        if (message.startsWith("/deleteroom ")) {
            if (DatabaseManager.isAdmin(nick)) {
                ...
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva na mazání místností.");
            }
            return;
        }
        ...
        if (message.startsWith("/kick ")) {
            if (DatabaseManager.isAdmin(nick)) {
                String[] p = message.substring(6).split(" ", 2);
                if (p.length == 2) {
                    Server.kickUser(p[0], p[1]);
                }
            } else {
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva na vyhození uživatelů.");
            }
            return;
        }
        ...
        if (message.startsWith("/mute ")) {
            if (DatabaseManager.isAdmin(nick)) {
                ...
                if (p.length == 3) {
                    try {
                        Server.muteUser(p[0], Integer.parseInt(p[1]), p[2]);
                    } catch (Exception e) {}
                }
            } else {
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva na umlčení uživatelů.");
            }
            return;
        }
        ...
        if (message.startsWith("/ban ")) {
            if (DatabaseManager.isAdmin(nick)) {
                ...
                if (p.length == 3) {
                    try {
                        Server.banUser(p[0], nick, p[2], Long.parseLong(p[1]));
                    } catch (Exception e) {}
                }
            } else {
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva na banování uživatelů.");
            }
            return;
        }
        ...
        if (message.startsWith("/broadcast ")) {
            if (DatabaseManager.isAdmin(nick)) {
                ...
                conn.send("MSG:0:SYSTEM:❌ Neoprávněný přístup. Nemáš práva posílat globální oznámení.");
            }
            return;
        }
    ...
    public void disconnectWebUser(String nick, String reason) {
        for (Map.Entry<WebSocket, String> entry : webClients.entrySet()) {
            if (entry.getValue().equalsIgnoreCase(nick)) {
                entry.getKey().send("DISCONNECT:" + reason);
    ...
    public void muteWebUser(String nick, int seconds) {
        for (Map.Entry<WebSocket, String> entry : webClients.entrySet()) {
            if (entry.getValue().equalsIgnoreCase(nick)) {
