...
public class DatabaseManager {
    ...
    public static void initDatabase() {
        ...
        try (Connection conn = connect(); Statement stmt = conn.createStatement()) {
            ...
            System.out.println("CRITICAL DB: " + e.getMessage());
        }
    }
    ...
    public static void muteUser(String username, long seconds) {
        long until = (seconds == -1) ? -1 : System.currentTimeMillis() + (seconds * 1000L);
        String sql = "UPDATE users SET muted_until = ? WHERE username = ?";
    ...
    public static boolean isMuted(String username) {
        String sql = "SELECT muted_until FROM users WHERE username = ?";
        try (Connection conn = connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, username);
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                long until = rs.getLong("muted_until");
    ...
    public static long getMuteRemaining(String username) {
        String sql = "SELECT muted_until FROM users WHERE username = ?";
        try (Connection conn = connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, username);
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                ...
                return remainMs > 0 ? (remainMs / 1000L) : 0;
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static int getUserLevel(String username) {
        ...
        try (Connection conn = connect(); PreparedStatement pstmt =prepareStatement(conn, sql)) {
            pstmt.setString(1, username);
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
