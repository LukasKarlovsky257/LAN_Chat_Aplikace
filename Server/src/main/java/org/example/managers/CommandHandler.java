...
public class CommandHandler {
        ...
        switch (cmd) {
            ...
            case "kick":
                if (client.isAdmin()) {
                    ...
                    if (kickParts.length > 0 && !kickParts[0].isEmpty()) {
                        String target = kickParts[0];
                        String reason = kickParts.length > 1 ? kickParts[1] : "Vyhozen administrátorem " + client.getClientName();
                        Server.kickUser(target, reason);
                    } else {
                        client.sendEncryptedMessage("MSG:0:SYSTEM:Použití: /kick Jméno [Důvod]");
                    }
                } else {
            ...
            case "ban":
                if (client.isAdmin()) {
                    String[] banParts = args.split(" ", 3);
                    if(banParts.length >= 2) {
                        String target = banParts[0];
                        try {
                            long seconds = Long.parseLong(banParts[1]);
                            String reason = banParts.length > 2 ? banParts[2] : "Porušení pravidel";
                            Server.banUser(target, client.getClientName(), reason, seconds);
                        } catch (NumberFormatException e) {
                            client.sendEncryptedMessage("MSG:0:SYSTEM:⚠️ Čas musí být číslo!");
                        }
                    } else {
                        client.sendEncryptedMessage("MSG:0:SYSTEM:Použití: /ban Jméno Čas [Důvod]");
                    }
                } else {
                    client.sendEncryptedMessage("MSG:0:SYSTEM:⛔ Nemáš práva admina!");
                }
    ...
    public static void sendHelp(ServerThread client) {
        ...
        if (client.isAdmin()) {
            ...
            client.sendEncryptedMessage("MSG:0:SYSTEM:/kick [nick] [důvod] - Vyhodit uživatele");
            client.sendEncryptedMessage("MSG:0:SYSTEM:/ban [nick] [sekundy] [důvod] - Zabanovat");
            client.sendEncryptedMessage("MSG:0:SYSTEM:/unban [nick] - Odblokovat");
            client.sendEncryptedMessage("MSG:0:SYSTEM:/mute [nick] [sekundy] [důvod] - Umlčet");
