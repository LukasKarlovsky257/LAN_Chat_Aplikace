...
public class Klient extends JFrame {
    ...
    public Klient() {
        super("Java Chat v8.4 (Rooms & Fast Launch)");
        setSize(950, 750);
        ...
        // Okno se načte hned, klíče se dopočítají, než uživatel vyplní heslo.
        new Thread(() -> {
            try {
    ...
    private void showRoomDialog() {
        String roomName = JOptionPane.showInputDialog(this,
                "Zadej název místnosti (nebo vytvoř novou):",
                "Výběr místnosti",
                JOptionPane.PLAIN_MESSAGE);

        if (roomName != null && !roomName.trim().isEmpty()) {
            try {
                out.println(CryptoUtils.encrypt("/join " + roomName.trim(), serverKey));
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Chyba při změně místnosti.");
            }
        }
    }
    ...
    private void addMessageBubble(int id, String sender, String text, String type, byte[] data) {
        ...
        switch (type) {
            ...
            case "FILE" -> {
                ...
                try {
                    if (!cachedFile.exists() && data != null) Files.write(cachedFile.toPath(), data);
                } catch (Exception e) {
                }
                ...
                saveBtn.addActionListener(e -> {
                    try {
                        ...
                        JOptionPane.showMessageDialog(this, "Nelze otevřít: " + ex.getMessage());
                    }
                });
                bubble.add(saveBtn);
                contentComp = saveBtn;
    ...
    private void playVideoWithJavaFX(String videoUrl, String title) {
        SwingUtilities.invokeLater(() -> {
            ...
            Platform.runLater(() -> {
                ...
                // iPhone User Agent pro obejití kodeků na Windows
    ...
    private void processIncomingMessage(String line) {
        try {
            ...
            // --- ZMĚNA MÍSTNOSTI ---
                ...
                messagesBox.repaint();
                chatInfoLabel.setText("Místnost: " + newRoomName);
                messageMap.clear();
