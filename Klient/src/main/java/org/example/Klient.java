...
public class Klient extends JFrame {
    ...
    public final Map<String, String> roomKeys = new ConcurrentHashMap<>();
    ...
    public void processMessage(String rawLine) {
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                if (line.startsWith("ROOM_KEY_REQ:")) {
                    String[] parts = line.substring(13).split(":", 3);
                    if (parts.length == 3) {
                        String requester = parts[0];
                        String roomName = parts[1];
                        String pubKeyBase64 = parts[2];
                        String myAesKey = roomKeys.get(roomName);
                        if (myAesKey != null) {
                            try {
                                PublicKey targetKey = CryptoUtils.getPublicKeyFromBytes(pubKeyBase64);
                                String encryptedAes = CryptoUtils.encrypt(myAesKey, targetKey);
                                network.sendEncryptedMessage("ROOM_KEY_RES:" + requester + ":" + encryptedAes);
                                System.out.println("🔐 Odeslán klíč pro místnost " + roomName + " uživateli " + requester);
                            } catch (Exception e) { e.printStackTrace(); }
                        }
                    }
                    return;
                ...
                if (line.startsWith("ROOM_KEY_RES:")) {
                    String[] parts = line.substring(13).split(":", 2);
                    if (parts.length == 2) {
                        String roomName = parts[0];
                        String encryptedAesKey = parts[1];
                        try {
                            String decryptedKey = CryptoUtils.decrypt(encryptedAesKey, this.myPrivateKey);
                            if (decryptedKey != null) {
                                roomKeys.put(roomName, decryptedKey);
                                this.chatPanel.addMessage(0, "SYSTEM", "🔒 Zabezpečené spojení navázáno (RSA/AES)", "TEXT", null);
                            }
                        } catch (Exception e) { e.printStackTrace(); }
                    }
                    return;
                ...
                if (line.startsWith("AUTH_OK") || line.startsWith("LOGIN_OK")) {
                    ...
                    String username = loginPanel.getUserField();

                    if ("admin".equals(username)) {
                        this.isAdmin = true;
                    } else if (parts.length >= 3) {
                        this.isAdmin = "true".equalsIgnoreCase(parts[2]);
                    } else {
                        this.isAdmin = line.contains("ADMIN:true");
                    }

                    this.myNick = username;
                    ...
                    String newRoom = line.substring(13);
                    this.chatPanel.clearChat();
                    this.chatPanel.updateHeader(newRoom, isAdmin);

                    if (!newRoom.equals("Lobby") && !roomKeys.containsKey(newRoom)) {
                        String myKeyB64 = CryptoUtils.publicKeyToString(this.myPublicKey);
                        this.network.sendEncryptedMessage("ROOM_KEY_REQ:" + newRoom + ":" + myKeyB64);
                    }
                }
                    ...
                    if (p.length == 3 && this.chatPanel != null) this.chatPanel.setTypingStatus(p[1], Integer.parseInt(p[2]));
                    return;
                }
            } catch (Exception e) { e.printStackTrace(); }
        });
