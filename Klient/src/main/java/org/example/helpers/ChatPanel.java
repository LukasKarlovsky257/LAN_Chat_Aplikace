...
public class ChatPanel extends JPanel {
    ...
    // 🔥 ABSOLUTNÍ ZBRAŇ PROTI GHOSTINGU (Maže stopy po překreslení u poloprůhledných panelů)
    ...
    public ChatPanel(Klient app) {
        ...
        this.setOpaque(true); // 🔥 POVINNÉ: Zabraňuje glitchům s hlavním oknem
        ...
        // 🔥 FIX PROTI DUCHŮM PŘI SCROLLOVÁNÍ NA POZADÍ:
        ...
        userScroll.getViewport().setScrollMode(JViewport.SIMPLE_SCROLL_MODE); // Oprava scrollování v listu lidí
        ...
        southContainer.add(this.typingLabel, BorderLayout.NORTH);

        // REPLY PANEL
        this.replyPreviewPanel = new TranslucentPanel(new BorderLayout());
        this.replyPreviewPanel.setBackground(new Color(0, 243, 255, 30));
        ...
        this.messageField.addKeyListener(new KeyAdapter() {
            @Override public void keyTyped(KeyEvent e) {
                ...
                if (!isTyping) { app.getNetwork().sendRawMessage("TYPING:1"); isTyping = true; }
            }
        });
    ...
    public void addMessage(int id, String sender, String rawText, String type, byte[] data) {
        SwingUtilities.invokeLater(() -> {
            ...
            if (type.equals("TEXT") && text != null && text.startsWith("REPLY|")) {
                ...
                if (parts.length == 4) { replySender = parts[1]; replyText = parts[2]; text = parts[3]; }
            }
            ...
            if (isSystem) {
                ...
                sysBubble.setBorder(new EmptyBorder(4, 12, 4, 12)); // Menší border, protože tu není avatar
            ...
            // 🔥 FIX AVATARŮ: Nastavení Borderu tak, aby udělal fyzické místo pro fotku a ta nepřekrývala text!
            RoundedBubblePanel bubble = new RoundedBubblePanel(new BorderLayout(), isMe, sender, app);
            if (isMe) {
                bubble.setBorder(new EmptyBorder(10, 14, 10, 55)); // 45px mezera napravo pro avatar!
            } else {
                bubble.setBorder(new EmptyBorder(10, 55, 10, 14)); // 45px mezera nalevo pro avatar!
            }
    ...
    public void setMuted(int seconds) {
        ...
        this.messageField.setBackground(new Color(255, 230, 230, 100));
    ...
    private void unmuteUI() {
        ...
        this.messageField.setBackground(ModernTheme.INPUT_BG);
    ...
    private void deactivatePrivateMode() { this.privateChatTarget = null; this.privateModePanel.setVisible(false); }
    private void cancelReply() { this.replyingToSender = null; this.replyingToText = null; this.replyPreviewPanel.setVisible(false); }
    ...
    public void setTypingStatus(String user, boolean typing) {
        ...
        SwingUtilities.invokeLater(() -> {
            if (typing) this.typingLabel.setText("✍️ " + user + " právě píše...");
    ...
    private void createContextMenu(JPanel bubble, JComponent content, int id, String sender, String type, String text, boolean isMe, boolean isSystem) {
        ...
        if (type.equals("TEXT")) {
            ...
            replyItem.addActionListener(e -> {
                this.replyingToSender = sender; this.replyingToText = text;
                ...
                this.replyPreviewPanel.setVisible(true); this.messageField.requestFocus();
            });
            menu.add(replyItem);
    ...
    // 🔥 FIX: Úprava Bubliny aby avatar nepřekrýval text a byl zarovnán
    private static class RoundedBubblePanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            // Kreslení samotné bubliny (vytváříme fyzické místo pro fotku posunutím)
            ...
            if (!isSys) {
                int size = 32;
                int x = isMe ? getWidth() - size : 0; // Pokud jsi to ty, avatar je napravo na kraji
                ...
                if (avatarImg != null) {
                    ...
                    // Záložní písmeno (když není profilovka)
